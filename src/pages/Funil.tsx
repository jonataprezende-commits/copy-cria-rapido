        ...
        <main className="flex-1 p-6 md:p-8 flex items-center justify-center">
          <div className="relative w-full max-w-2xl">
            <div className="blur-sm pointer-events-none select-none bg-card rounded-lg shadow-premium p-8">
              <h2 className="text-2xl font-bold text-foreground mb-2">Funil completo em 1 clique</h2>
              <p className="text-muted-foreground">Do anúncio à venda. A IA monta todo o funil do seu produto.</p>
            </div>
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <main className="flex-1 p-6 md:p-8 overflow-auto">
        ...
        <p className="text-sm text-muted-foreground mb-6">Do anúncio à venda. A IA monta todo o funil do seu produto.</p>
        ...
        {!results ? (
          <div className="max-w-lg mx-auto bg-card rounded-lg shadow-premium p-6 space-y-4">
            ...
            <div>
              ...
              <Textarea value={description} onChange={(e) => setDescription(e.target.value)} className="mt-1.5 bg-background resize-none" rows={3} required />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input value={price} onChange={(e) => setPrice(e.target.value)} placeholder="ex: 497" className="mt-1.5 h-11 bg-background" />
              </div>
              <div>
            ...
            <Button onClick={handleGenerate} disabled={loading || !productName.trim()} className="w-full h-12 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold text-base">
          ...
          <Tabs defaultValue="anuncios">
            <TabsList className="mb-4">
              ...
              <TabsTrigger value="script">Script de Vídeo</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="landing">
              {results.landing && (
                <div className="bg-card rounded-lg shadow-premium p-6 space-y-6">
                  ...
                  <Button variant="outline" onClick={() => handleCopySection(JSON.stringify(results.landing, null, 2))} className="text-foreground">
            ...
            <TabsContent value="emails" className="space-y-3">
              {results.emails?.map((email: any, i: number) => (
                <div key={i} className="bg-card rounded-lg shadow-premium p-5">
                  <p className="text-xs text-muted-foreground mb-1">E-mail {i + 1} — {email.tema}</p>
            ...
            <TabsContent value="script">
              {results.script && (
                <div className="bg-card rounded-lg shadow-premium p-6 space-y-4">
                  {results.script.map((segment: any, i: number) => (
